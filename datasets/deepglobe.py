...
class_list = ['urban', 'agriculture', 'rangeland', 'forest', 'water', 'barren','unknown']
...
def load_cls_label_list(name_list_dir):
    label_dict = np.load(os.path.join(name_list_dir, 'cls_labels_onehot_deepglobe_revised.npy'), allow_pickle=True).item()
    modified_dict = {}
    for key, value in label_dict.items():
        modified_dict[key] = value # 提取除第一个元素外的所有元素
...
class deepglobeClsDataset(deepglobeDataset):
    def __init__(self,
                 root_dir=None,
                 name_list_dir=None,
                 split='train',
...
class deepglobeSegDataset(deepglobeDataset):
    def __init__(self,
                 root_dir=None,
                 name_list_dir=None,
                 split='train',
