...
#     parser.add_argument('--dataset_path',default=r'D:\BaiduNetdiskDownload\Potsdam',
#                         help='potsdam folder path')
#     parser.add_argument('--tmp_dir', default=r'D:\BaiduNetdiskDownload\Potsdam\temp',
...
#         if args.noboundary:
#             out_dir = osp.join(r'D:\BaiduNetdiskDownload\Potsdam', f'potsdam_IRRG_noB_{args.clip_size}_{args.stride_size}')
#         else:
#             out_dir = osp.join(r'D:\BaiduNetdiskDownload\Potsdam', f'potsdam_IRRG_wiB_{args.clip_size}_{args.stride_size}')
#     else:
...
#                     clip_big_image(src_path, dst_dir, args, 'test', to_label=True)
#                     dst_dir_color = osp.join(out_dir, 'ann_dir', data_type+'_color')
#                     clip_big_image(src_path, dst_dir_color, args, 'test', to_label=False)
#                 else:
#                     dst_dir = osp.join(out_dir, 'img_dir', data_type)
#                     clip_big_image(src_path, dst_dir, args, 'test', to_label=False)
#                 prog_bar.update()
#
...
# 指定文件夹路径
base_folder = r'D:\BaiduNetdiskDownload\DeepGlobe_Land\deepglobe_512_512\img_dir'
train_folder = os.path.join(base_folder, 'train')
test_folder = os.path.join(base_folder, 'test')


# 函数来提取文件名并写入文件
def extract_and_save(folder_path, output_file):
    image_names = []
...
# 保存train文件名
train_output_file = os.path.join(base_folder, 'train.txt')
extract_and_save(train_folder, train_output_file)
print(f"Saved train image names to {train_output_file}")

# 保存test文件名
test_output_file = os.path.join(base_folder, 'test.txt')
extract_and_save(test_folder, test_output_file)
print(f"Saved test image names to {test_output_file}")
