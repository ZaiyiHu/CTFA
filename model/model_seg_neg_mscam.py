...
class network(nn.Module):
    ...
    def forward(self, x, cam_only=False, crops=None, n_iter=None):
        ...
        _x1 = F.interpolate(_x4, size=(int(h * 0.5), int(w * 0.5)), mode='bilinear',
                            align_corners=False)
        _x2 = F.interpolate(_x4, size=(int(h * 1.5), int(w * 1.5)), mode='bilinear',
                            align_corners=False)
        _x3 = F.interpolate(_x4, size=(int(h * 2), int(w * 2)), mode='bilinear', align_corners=False)
        _x_aux = self.to_2D(x_aux, h, w)

        seg = self.decoder(_x4)

        if cam_only:
            cam_x1 = F.conv2d(_x1, self.classifier.weight).detach()
            cam_x2 = F.conv2d(_x2, self.classifier.weight).detach()
            cam_x3 = F.conv2d(_x3, self.classifier.weight).detach()
            cam_x4 = F.conv2d(_x4, self.classifier.weight).detach()
            cam_x1 = F.interpolate(cam_x1, size=(int(cam_x4.shape[2]), int(cam_x4.shape[3])), mode='bilinear',
                                 align_corners=False)
            cam_x2 = F.interpolate(cam_x2, size=(int(cam_x4.shape[2]), int(cam_x4.shape[3])), mode='bilinear',
                                 align_corners=False)
            cam_x3 = F.interpolate(cam_x3, size=(int(cam_x4.shape[2]), int(cam_x4.shape[3])), mode='bilinear',
                                 align_corners=False)

            cam = (cam_x1 + cam_x2 + cam_x3 +cam_x4)/2
