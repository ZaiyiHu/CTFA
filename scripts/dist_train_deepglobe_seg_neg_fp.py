...
parser.add_argument("--work_dir", default="work_dir_deepglobe_wseg", type=str, help="work_dir_deepglobe_wseg")
...
parser.add_argument("--w_joi", default=0.2, type=float, help="w_joi")
...
def validate(model=None, data_loader=None, args=None):
    ...
    seg_score = evaluate.scores(gts, preds,7)
    cam_score = evaluate.scores(gts, cams,7)
    cam_aux_score = evaluate.scores(gts, cams_aux,7)
    model.train()

    tab_results,mIoU = format_tabs([cam_score, cam_aux_score, seg_score], name_list=["CAM", "aux_CAM", "Seg_Pred"], cat_list=deepglobe.class_list)
...
def train(args=None):
    ...
    train_dataset = deepglobe.deepglobeClsDataset(
