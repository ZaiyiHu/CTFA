...
def validate(model=None, data_loader=None, args=None):
    ...
    tab_results, mIoU = format_tabs([seg_score], name_list=["Seg_Pred"], cat_list=potsdam.class_list)
...
def train(args=None):
    ...
    param_groups = [
        {'params': param_groups[0], 'lr': lr_multi[0] * 0.0005, 'weight_decay': 0.0005},
        {'params': param_groups[1], 'lr': lr_multi[1] * 0.0005, 'weight_decay': 0},
        {'params': param_groups[2], 'lr': lr_multi[2] * 0.0005, 'weight_decay': 0.0005},
        {'params': param_groups[3], 'lr': lr_multi[3] * 0.0005, 'weight_decay': 0},
    ]
    model.to(device)
    # optimizer
    ...
    for n_iter in range(args.max_iters):
        ...
        if n_iter <= 2000:
            loss = 1.0 * cls_loss + 4.0 * reg_loss1 + 4.0 * reg_loss2 + 0.0 * seg_loss
        else:
        ...
        if (n_iter + 1) % args.eval_iters == 0:
            ...
            if args.save_ckpt and (n_iter + 1) >= 10000 and mIoU_result > mIoU:
                mIoU = mIoU_result
                ckpt_name = os.path.join(args.ckpt_dir, "Best mIoU: {}, model: {} model_iter_%d.pth".format(mIoU,"SLRNet_potsdam") % (n_iter + 1))
